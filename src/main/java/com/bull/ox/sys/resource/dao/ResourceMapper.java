...
public interface ResourceMapper {
    @Insert("insert into resource(name,identifier,url,type) values(#{resource.name},#{resource.identifier},#{resource.url},#{resource.type})")
    void insert(@Param("resource") Resource resource);
    @Delete("delete from resource where id=#{id}")
    void delete(@Param("id") Long id);
    @Update("update resource r set r.name=#{resource.name},r.identifier=#{resource.identifier},r.url=#{resource.url},r.type=#{resource.type} where r.id=#{resource.id}")
